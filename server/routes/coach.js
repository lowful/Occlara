...
NO ARTICLE WHEN THE AGENT IS THE PERSON. Write "Reyna is holding B Main", not "a Reyna is holding B Main", and never "the Reyna". There is one of each agent in a match, so the name is already specific, and the overlay replaces the name with that agent's portrait, glowing in a colour that says whose side they are on. An article reads as clutter beside the portrait, and the voice coach says it out loud. This is about the agent as a PERSON. When the name modifies an object it keeps its article and its normal grammar: "a Sage wall", "a Viper orb", "the Sova dart" are all correct and must not be stripped. The no-agent fallback also keeps its article, because "an enemy" and "your teammate" are not names.

NAME THE KILLER, NOT THEIR EQUIPMENT. In a death review, say who killed the player: "You died to Sage before you could see her", not "you died to a Sage wall". The overlay only swaps a name for a portrait when the sentence is about the agent themselves, so naming a piece of their utility instead spends the one thing the player recognises at a glance. Say what the utility did afterwards, in its own clause, if it matters: "You died to Sage, who walled off your cover first."
...
- Ultimate: ${ctx.playerUlt === 'ready' ? 'READY' : ctx.playerUlt === 'charging' ? 'still charging' : 'Unknown, so do not mention it'}
...
STATE: {"side":"attack","phase":"active","round":5,"clock":"1:12","team":3,"enemy":1,"credits":4200,"hp":87,"alive":true,"aliveTell":"own rifle and HP 87 bottom center","mates":3,"foes":2,"weapon":"Vandal","ult":null,"map":"Ascent","mode":null,"mmPos":[0.48,0.2],"locLabel":"Mid Top","playerSpot":null,"enemySpot":null,"push":null,"pushOnSite":null,"spike":null,"spikeSpot":null,"killFeed":null,"teamRead":null,"note":null}
...
- ult: whether the player's ULTIMATE is ready, read from the ultimate icon and its point pips beside the ability row at the bottom, or from the X key prompt when it lights up. Report "ready" only when it is clearly charged and usable, "charging" when the pips are visibly incomplete, and null when you cannot tell. This is the one ability whose state decides whether a whole round plan is available, so it is worth a look, but a guess here is worse than a null: telling a player to ult with no ult is the fastest way to lose them.
...
function mapState(s) {
  ...
  // An enum, not free text, so an ambiguous read lands as absent rather than as
  // a third state nothing downstream knows how to treat.
  if (str(s.ult)) {
    const u = str(s.ult).toLowerCase();
    if (u === 'ready' || u === 'charging') out.playerUlt = u;
  }
