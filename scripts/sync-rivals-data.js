...
/*
 * A SECOND, SMALLER COPY FOR THE CLIENT, and it is not duplication for
 * convenience.
 *
 * package.json ships "src/**", "assets/**", "node_modules/**" and package.json.
 * It does NOT ship server/. So a guard in src/ requiring the file above
 * resolves perfectly in the repo, passes every test, and throws MODULE_NOT_FOUND
 * the first time a real install reaches it. That is the exact mirror of the rule
 * check:server enforces in the other direction, and it fails later and more
 * quietly, because the only machine that reproduces it is a customer's.
 *
 * This copy carries hero to ability names and nothing else. The ability gate is
 * the only client side consumer and needs no health, no speed and no prose, and
 * a file that carries only what is used cannot drift in the parts that are not.
 */
const OUT_CLIENT = path.join(__dirname, '..', 'src', 'shared', 'rivals-abilities.generated.json');
...
function parseAbilities(html, heroName) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    // THE SITE'S OWN SPACER ROW, which is a real ability row in every structural
    // sense and names no ability. It reads 占位空格, "placeholder space", left in
    // from the Chinese build, and it carries an index, a name cell and an empty
    // description like any other row, so nothing about the shape rejects it. It
    // appeared on ALL 54 heroes, which made it look like a legitimate universal
    // ability rather than debris, and it would have become the one string every
    // hero in the game shares. Anything outside the Latin range is the same kind
    // of leftover: no English ability name has a CJK character in it.
    ...
    // ROW 0 AGAIN, when the name cell does not repeat the hero's full name.
    // Gorr the God Butcher's Base Stats row is labelled just "Gorr", so the
    // equality test above lets it through and "Gorr" became an ability whose
    // description was "175 Health + 175 Regenerative Shield". A prefix test
    // would catch it and would also throw away Cloak and Dagger, which are real
    // form-swap abilities on Cloak & Dagger. The description is the honest
    // signal: exactly one row in 612 describes itself as a health pool, and no
    // real ability text opens with a number followed by Health.
    if (/^\s*\d+\s*Health\b/i.test(desc)) continue;
...
async function main() {
  ...
  // The client copy, written in the same breath so the two cannot fall out of
  // step. See OUT_CLIENT for why it exists at all.
  //
  // IT CARRIES THE HAND WRITTEN TRAITS TOO, read out of rivals-heroes.js rather
  // than typed again here. Archetype, aim and air are community vocabulary that
  // appears on no official page, so they cannot be fetched, and the post match
  // review needs them to say what a hero is FOR. The alternative was a second
  // hand maintained copy in src/shared, which is how spectate-tells.js and
  // coach.js ended up mirroring each other by hand and needing a test to prove
  // they still agree. Generating it means there is nothing to keep in step.
  //
  // scripts/ may require from server/ freely. Only src/ may not, because only
  // src/ ships.
  const table = require(path.join(__dirname, '..', 'server', 'services', 'rivals-heroes.js'));
  const traits = {};
  for (const h of heroes) {
    ...
    traits[h.name] = t
      ...
      // cannot tell that from a hero the sync missed.
      : null;
  }
  ...
  const client = {
    ...
    source: INDEX,
    note: 'Generated by npm run sync:rivals. Do not hand edit.',
    abilities: Object.fromEntries(heroes.map((h) => [h.name, h.abilities.map((a) => a.name)])),
    traits,
  };
  fs.writeFileSync(OUT_CLIENT, JSON.stringify(client, null, 2));
  const rows = Object.values(client.abilities).reduce((n, a) => n + a.length, 0);
  console.log(`wrote ${OUT_CLIENT}`);
  console.log(`  ${Object.keys(client.abilities).length} heroes, ${rows} ability names`);
}
